...
class MLP(nn.Module):
    def __init__(self, chan, chan_out=256, inner_dim=2048):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(chan, inner_dim),
                                 nn.BatchNorm1d(inner_dim), nn.ReLU(),
...
class PPM(nn.Module):
    def __init__(self, *, chan, num_layers=1, gamma=2):
        ...
        if num_layers == 0:
            ...
            self.transform_net = nn.Sequential(nn.Conv2d(chan, chan, 1),
    ...
    def forward(self, x):
        ...
        out = einsum('b x y h w, b c h w -> b c x y', similarity,
                     transform_out)
        return out
