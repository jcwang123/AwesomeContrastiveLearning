...
class SimCLR(nn.Module):
    ...
    def forward(self, x, return_embedding=False):
        ...
        # [2*B, 2*B-1]
        sim_matrix2 = sim_matrix.masked_select(mask).view(2 * batch_size, -1)

        # compute loss
        pos_sim = torch.exp(
            torch.sum(online_proj_one * online_proj_two, dim=-1) /
            self.temperature)

        # [2*B]
        pos_sim = torch.cat([pos_sim, pos_sim], dim=0)
        loss = (-torch.log(pos_sim / sim_matrix2.sum(dim=-1))).mean()
        return loss
...
if __name__ == '__main__':
    ...
    resnet = resnet50()
    learner = SimCLR(resnet, image_size=128, hidden_layer='avgpool')
